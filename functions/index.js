...
exports.addJsonData = onRequest(async (req, res) => {
    ...
    try {
        ...
        let key = dayjs(gmail.date).utc().utcOffset(330)
            .format("DD MMM YY, hh:mm A") + " " + gmail.vendor.slice(0, 10);

        const docRef = db.collection("gmail").doc(key);

        // setDoc is now using the admin SDK's db instance
        docRef.set(gmail).then(() => {
            console.log("Executed setDoc");
            res.send("Executed setDoc"); // Make sure to send response after async operation
        }).catch(err => {
            console.error("Error in addJsonData setDoc:", err);
            res.status(500).send("Error - " + err.message);
        });

    } catch (err) {
        console.error("Error in addJsonData:", err);
        res.status(500).send("Error - " + err.message);
    }
