...
This guide will walk you through setting up the project on a Linux/Microsoft/MacOs environment.
...
- ** With VS Code:**
  ...
  4. Repository URL mentioned below
   ```bash
      https://github.com/rushikc/firebase-gmail-kickstart.git 
   ```
  ![VSCODE](public/docs/pics/vscode.png)

- **With Git:**
  ```bash
  git clone https://github.com/rushikc/firebase-gmail-kickstart.git
  ```
- **Direct Download:**
  Alternatively, you can download the project as a ZIP file from the repository's main page and extract it.
  go to https://github.com/rushikc/firebase-gmail-kickstart, click on Code button, then click on Download ZIP.
...
2.  **Navigate to Directory:** Use the `cd` command. For example, if the project is in your `Downloads` folder, run:
    ```cmd
    cd C:\\Users\\YourUsername\\Downloads\\firebase-gmail-kickstart
    ```
...
2. **Create a New Firebase Project:**
    ...
    - Enter a unique name for your project, this will be your project-id (e.g., `hello-2a`, `gmail-app-3455`).
...
4. **Upgrade to the Blaze Plan:**
    - By default, your project will be on the "Spark" plan. To enable all features required for FirebaseGmail, you must
...
- **Set up Environment Files:**
  ...
  2. **Create  the `env.js` file in the appScript folder:**
     ...
     const USER_ID = 'FirebaseGmail'; // Replace with your actual user ID
     const PROJECT_REGION = 'us-central1';
     
     ```
     - Replace `your-project-id` with your actual Firebase project ID.
     - Replace `FirebaseGmail` with your user if , it will be used in  JSON in firestore,  you can add your name, nickname or anything which you want to use as user id.
...
1. **Enable the Google Apps Script API:**
    ...
    - Navigate to `Profile` and ensure the **Google Apps Script API** is enabled.
...
5. **Create a New Apps Script Project:**
    ...
    ```bash
    clasp create --title "FirebaseGmail App Script" --type standalone
    ```
...
7. **Run the Apps Script Files:**
    ...
    - Click on the `FirebaseGmail App Script` project you just created.
    - In the left sidebar, you will see the files `emailParser.gs` and `trigger.gs`.
    ...
    - Once you select the mail, you will get a warning page,click on advanced & then click on `Go to FirebaseGmail App Script (unsafe)`.
    ...
    - Now click on `emailParser.gs` in the left sidebar to open it.
    - Click the **Run** button (▶️) to execute the script.
    - This will fetch the last 100 emails from your Gmail account, scan them for text data, and if any matches are found, it will add them to Firestore.

---

## Post-Setup Instructions

Congratulations! You have successfully completed the setup for FirebaseGmail.
...
For a more native experience, you can install FirebaseGmail as a Progressive Web App (PWA) directly from Google Chrome on both Android and iOS devices.
